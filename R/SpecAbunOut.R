...
function(data, 
  ...
  if (method == "all") {
    ...
    e <- SpecAbunAce(data, k, conf)
    f <- SpecAbunAce1(data, k, conf)
    g <- SpecAbunJack1(data, k, conf)
    h <- SpecAbunJack2(data, k, conf) 
  ...
  if (method == "Chao")
    out <- rbind(SpecAbunChao1(data, k, conf), SpecAbunChao1bc(data, k, conf))
  if (method == "CE")
